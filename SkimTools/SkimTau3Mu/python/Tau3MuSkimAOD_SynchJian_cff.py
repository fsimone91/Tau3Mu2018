...
looseMuons = cms.EDFilter("PATMuonSelector",
                          ...
                          cut = cms.string('abs(eta)<2.4 && (innerTrack().isNonnull) && (charge!=0) && (innerTrack().hitPattern().numberOfValidPixelHits()>0)'),
...
GlobalMuons = cms.EDFilter("PATMuonSelector",
                          src = cms.InputTag("looseMuons"),
                           cut = cms.string('pt > 2 &&  (isGlobalMuon()) > 0'),
                          filter = cms.bool(True)
) 

ThreeGlobalMuonsFilter = cms.EDFilter("CandViewCountFilter",
                             src = cms.InputTag("GlobalMuons"),
                             minNumber = cms.uint32(3),
                             #filter = cms.bool(True)                                                                                                                        
)

ThreeGlobalMuonsCand = cms.EDProducer("CandViewShallowCloneCombiner",
                         checkCharge = cms.bool(False),
                         cut = cms.string('(abs(charge)=1)'),              
                         decay = cms.string("GlobalMuons GlobalMuons GlobalMuons")
)

ThreeGlobalMuonsCandFilter = cms.EDFilter("CandViewCountFilter",
                                    src = cms.InputTag("ThreeGlobalMuonsCand"),
                                    minNumber = cms.uint32(1),
                                    #filter = cms.bool(True)                                                                                                                 
)
...
PlotsAfterTauCand = cms.EDAnalyzer('RecoMuonAnalyzer',
                                   muonsInputTag = cms.InputTag("GlobalMuons"),
                                   )
...
PlotsAfterGlbMuons= cms.EDAnalyzer('RecoMuonAnalyzer',
                                   muonsInputTag = cms.InputTag("GlobalMuons"),
                                   )

PlotsAfter3GlbMuons= cms.EDAnalyzer('RecoMuonAnalyzer',
                                   muonsInputTag = cms.InputTag("GlobalMuons"),
                                   )

PlotsAfter3GlobalMuonsCand= cms.EDAnalyzer('RecoMuonAnalyzer',
                                   muonsInputTag = cms.InputTag("GlobalMuons"),
                                   )
...
ThreeMuonSelSeq = cms.Sequence(InitialPlots *
                               ...
                               GlobalMuons*
                               PlotsAfterGlbMuons*
                               ThreeGlobalMuonsFilter*
                               PlotsAfter3GlbMuons*
                               ThreeGlobalMuonsCand*       
                               ThreeGlobalMuonsCandFilter*
                               PlotsAfterTauCand *
                               #ThreeMuonsVtxKinFit *
