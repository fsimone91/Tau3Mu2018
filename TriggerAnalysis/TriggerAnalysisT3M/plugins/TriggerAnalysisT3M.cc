// -*- C++ -*-
//
// Package:    TriggerAnalysis/TriggerAnalysisT3M
// Class:      TriggerAnalysisT3M
//
/**\class TriggerAnalysisT3M TriggerAnalysisT3M.cc MiniAna2017/TriggerAnalysisT3M/plugins/TriggerAnalysisT3M.cc
...
class TriggerAnalysisT3M : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
public:
    explicit TriggerAnalysisT3M(const edm::ParameterSet&);
    ~TriggerAnalysisT3M();
...
private:
  ...
  std::vector<double> L1Muon_Pt, L1Muon_Eta, L1Muon_Phi,L1Muon_BX,L1Muon_Quality,L1Muon_Charge,L1Muon_ChargeValid,L1Muon_TfMuonIndex,L1Muon_dPhi,L1Muon_dEta,L1Muon_rank,L1Muon_isoSum;	
    ...
    TriggerAnalysisT3M::TriggerAnalysisT3M(const edm::ParameterSet& iConfig){
        edm::InputTag algInputTag_;
    ...
    TriggerAnalysisT3M::~TriggerAnalysisT3M()
    {
    ...
    float TriggerAnalysisT3M::dR(float eta1, float eta2, float phi1, float phi2){
    ...
    float TriggerAnalysisT3M::dRtriggerMatch(pat::Muon m, trigger::TriggerObjectCollection triggerObjects) {
        float dRmin = 1.;                                                                          
...
float TriggerAnalysisT3M::dPtTriggerMatch(pat::Muon m, trigger::TriggerObjectCollection triggerObjects) {
  float dPtmin = 10000.;
  for (unsigned int i = 0 ; i < triggerObjects.size() ; i++) {
  ...
  return dPtmin;
    ...
    void TriggerAnalysisT3M::beginRun(edm::Run const& iRun, edm::EventSetup const& iSetup, const edm::Event& iEvent) {
      /*  
    ...
    void
    TriggerAnalysisT3M::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
    {
        using namespace edm;
        using namespace std;
	...
	for (int ibx = gmuons->getFirstBX(); ibx <= gmuons->getLastBX(); ++ibx) {
	  for (auto itr = gmuons->begin(ibx); itr != gmuons->end(ibx); ++itr) {
	    ...
	    //L1Muon_Quality.push_back(itr->qual());
	    L1Muon_Charge.push_back(itr->charge());
	    //L1Muon_ChargeValid.push_back(itr->chargeValid());
	    ...
	    //L1Muon_isoSum.push_back(itr->L1Muon_isoSum());
	    cout << "Muon : "
	...
	*/
	...
	if (!iEvent.isRealData())
	  {
		...
		if(l1tName.find("DoubleMu") != string::npos || l1tName.find("TripleMu") != string::npos || l1tName.find("SingleMu")){
		  if(initialDecisions.at(i_l1t).second){
		  cout<<"MC: L1 name="<<l1tName<<" decision="<<initialDecisions.at(i_l1t).second<<endl;
		  }
	...
	for (size_t i_hlt = 0; i_hlt != triggerResults->size(); ++i_hlt){
	    ...
	    if(hltName.find("HLT_DoubleMu") != string::npos){
	      if(hltName.find("HLT_DoubleMu3_TkMu_DsTau3Mu")){
	...
	for (unsigned int i = 0 ; i <MuonsObjects.size() ; i++) {
	  ...
	  cout<<"HLT Objects: pt="<<MuonsObjects[i].pt()<<" eta="<<MuonsObjects[i].eta()<<" phi="<<MuonsObjects[i].phi()<<endl;
	}
	  ...
	  const pat::Muon *mu3 = dynamic_cast<const pat::Muon *>(c3);
	    ...
	    float dPtRel1 = 999., dPtRel2 = 999., dPtRel3 = 999.;
            ...
            //save the trigger objetcs corresponding to muon leg
                ...
                dR1 = TriggerAnalysisT3M::dRtriggerMatch(*mu1, MuonsObjects);
                dR2 = TriggerAnalysisT3M::dRtriggerMatch(*mu2, MuonsObjects);
                dR3 = TriggerAnalysisT3M::dRtriggerMatch(*mu3, MuonsObjects);
		dPtRel1 = TriggerAnalysisT3M::dPtTriggerMatch(*mu1, MuonsObjects);
		dPtRel2 = TriggerAnalysisT3M::dPtTriggerMatch(*mu2, MuonsObjects);
		dPtRel3 = TriggerAnalysisT3M::dPtTriggerMatch(*mu3, MuonsObjects);
	...
	uint k=0;        
	    ...
	    Muon_simType.push_back(mu->simExtType());
	    Muon_simBX.push_back(mu->simBX());
	    //	    Muon_simTpEvent.push_back(mu->simTpEvent());
	    //Muon_simMatchQuality.push_back(mu->simMatchQuality());
	    //cout<<"simType="<<mu->simExtType()<<endl;
	
            //Vtx position
            Muon_vx.push_back(mu->vx());
            Muon_vy.push_back(mu->vy());
            Muon_vz.push_back(mu->vz());
	    Muon_IP2D_BS.push_back(mu->dB(pat::Muon::BS2D));
	    Muon_IP3D_BS.push_back(mu->dB(pat::Muon::BS3D));
	    Muon_IP2D_PV.push_back(mu->dB(pat::Muon::PV2D));
	    Muon_IP3D_PV.push_back(mu->dB(pat::Muon::PV3D));
	    //Muon_DZ_PV.push_back(mu->dB(pat::Muon::PVDZ));
	    //typedef enum IPTYPE { PV2D = 0, PV3D = 1, BS2D = 2, BS3D = 3, PVDZ = 4, IpTypeSize = 5 } IpType;
	    //Cout<<"IP from BS"<<mu->dB(pat::Muon::BS2D)<<endl;
	    ...
	    //	    MuonLowPtMVA_Val.push_back(mu->lowptMvaValue());
    ...
    void TriggerAnalysisT3M::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
    ...
    DEFINE_FWK_MODULE(TriggerAnalysisT3M);
